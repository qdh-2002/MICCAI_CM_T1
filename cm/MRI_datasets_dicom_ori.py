...
class MRIDicomDataset(Dataset):
    def __init__(self, root_path, sequence="AX_T2", crop=288, scale=4.0, image_size=320, select_k=None,
        ...
        print(f"🧠 Loading dataset from: {root_path} | Looking for sequence: '{sequence}'")
        ...
        for patient_id in tqdm(patient_dirs, desc="🔍 Scanning patients"):
            patient_path = os.path.join(root_path, patient_id)
            if not os.path.isdir(patient_path):
                continue

            # 🔍 Fuzzy match sequence folder (e.g., AX_DIFFUSION_ADC)
            sequence_dirs = [d for d in os.listdir(patient_path) if sequence in d]
            if not sequence_dirs:
                print(f"⚠️  Patient {patient_id}: no sequence matching '{sequence}' found.")
                continue

            for seq in sequence_dirs:
                seq_path = os.path.join(patient_path, seq)
                print(f"📂 Reading from: {seq_path}")
                slices = []
                ...
                if len(slices) == 0:
                    print(f"⚠️  No valid slices in {seq_path}")
                    continue
        ...
        print(f"✅ Dataset initialized with {len(self.imgs)} total images.")

        if len(self.imgs) == 0:
            raise RuntimeError(f"No usable DICOM images found matching '{sequence}' in: {root_path}")
    ...
    def __getitem__(self, idx):
        ...
        hr_inte = F.interpolate(lr.unsqueeze(0), size=(self.image_size, self.image_size), mode='bilinear', align_corners=True).squeeze(0)
