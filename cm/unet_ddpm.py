...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, time_emb):
        b, c, h, w = x.shape
        h = self.block1(x)
        h = self.noise_func(h, time_emb).type(x.dtype)
        h = self.block2(h)
...
class UNet(nn.Module):
    def __init__(
        ...
        # coord conv
        # self.coord_enocding = PositionalEncoding(inner_channel)
        self.coord_conv = nn.Sequential(
            ...
            nn.Conv2d(3, inner_channel, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv2d(inner_channel, inner_channel, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv2d(inner_channel, 1, kernel_size=3, padding=1) 
        )
        ###
    ...
    def forward(self, x, timesteps, hr_inte, lr_coords):
        ...
        coord_features = self.coord_conv(coords_resized)
        x = torch.cat([x, hr_inte, coord_features], dim=1)
...
#         image_size=2400
# ).cuda()
# x = torch.randn((1,2,832,2400)).cuda()
# lr_coords = torch.randn((1,3,832,300)).cuda()
# time = torch.randn((1,1)).cuda()
# with torch.no_grad():
#     out = model(x, lr_coords, time)