...
class UNetModel(nn.Module):
    ...
    now conditioned on both an intensity map and a sampling-vector.
    """
    ...
    def __init__(
        ...
        # First conv must accept: in_channels + 1 (hr_inte) + sampling_channels
        first_in = in_channels + 1 + sampling_channels
        ...
        # Final conv back to out_channels
    ...
    def forward(self, x, timesteps, hr_inte, sampling_vec, y=None):
        """
        Apply the model to an input batch, conditioned on intensity map and sampling one-hot.
        """
        B, _, H, W = x.shape
        # Build sampling map from [B, sampling_channels]
        samp_map = sampling_vec.view(B, self.sampling_channels, 1, 1).expand(B, self.sampling_channels, H, W)
        # Concatenate x, hr_inte (1 channel), and samp_map
        x = th.cat([x, hr_inte, samp_map], dim=1)
        h = x.type(self.dtype)
